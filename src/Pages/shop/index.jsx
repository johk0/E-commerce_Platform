...
const Shop = () => {
  ...
  const [searchedData, setSearchedData] = useState(demoProducts);
  ...
  useEffect(() => {
    ...
    if (selectedSearch.length > 0) {
      filteredData = searchProductsByName(demoProducts, selectedSearch);
    }
        ...
        <div className="flex items-center gap-2 mx-3">
          {Array.from({ length: numPages }, (_, index) => (
            <button
              ...
              className={`p-2 ${
                currentPage === index + 1 ? "bg-blue-500 text-white rounded" : "bg-gray-300 rounded"
              }`}>
              {index + 1}
            </button>
          ))}
