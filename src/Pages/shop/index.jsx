...
const Shop = () => {
  ...
  return (
    <div className="p-4 px-7">
      <header className="flex justify-between items-center mb-4 max-md:flex-col">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-2 max-md:hidden ${view === 'grid' ? 'text-blue-500' : ''}`}
          >
            <FontAwesomeIcon icon={faThLarge} />
          </button>
          <button
            onClick={() => setView('list')}
            className={`p-2 max-md:hidden ${view === 'list' ? 'text-blue-500' : ''}`}
          >
      ...
      <div className={`grid ${view === 'grid' ? 'grid-cols-[repeat(auto-fill,_minmax(380px,_1fr))] max-sm:grid-cols-[repeat(auto-fill,_minmax(320px,_1fr))] gap-6' : 'grid-cols-1 gap-4'}`}>
        {sortedProducts
      ...
      <div className="flex justify-center items-center mt-4 max-w-80 m-auto">
        <button onClick={() => handleChangeOfPage(1)} className="p-2 bg-gray-300 rounded">
          First
        </button>
        <div className="flex items-center gap-2 mx-3">
          {Array.from({ length: numPages }, (_, index) => (
            <button
              ...
              className={`p-2 ${currentPage === index + 1 ? 'bg-blue-500 text-white rounded' : 'bg-gray-300 rounded'}`}
            >
              {index + 1}
            </button>
          ))}
